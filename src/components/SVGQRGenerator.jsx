...
const SVGQRGenerator = () => {
  ...
  const generateSVGQR = (data) => {
    ...
    // Use QR Server API with SVG format - more reliable
    const encodedData = encodeURIComponent(data);
    const svgUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodedData}&format=svg&margin=2`;
    
    // For display, we'll use the URL directly since it's an SVG
    setQrCodeSvg(svgUrl);
  };
  ...
  const downloadSVG = () => {
    ...
    // Since qrCodeSvg is now a URL, we need to fetch it first
    fetch(qrCodeSvg)
      ...
      .catch(error => {
        ...
        window.open(qrCodeSvg, '_blank');
      });
  };

  useEffect(() => {
    if (qrData) {
      generateSVGQR(qrData);
    }
  }, [qrData]);
    ...
    <Container>
      <Card>
        ...
        <div style={{ marginBottom: '2rem' }}>
          ...
          />
          <Button onClick={() => generateSVGQR(qrData)}>
        ...
        {qrCodeSvg && (
          <QRCodeContainer>
            <h3 style={{ color: '#FFB000', marginBottom: '1rem' }}>Generated SVG QR Code:</h3>
            <img 
              src={qrCodeSvg} 
              alt="QR Code" 
              style={{ 
