-- Migration: Enhance avatar storage policies and ensure proper database access
-- Run this in your Supabase SQL editor

-- Ensure avatar_url column exists and has proper constraints
ALTER TABLE public.user_profiles 
ADD COLUMN IF NOT EXISTS avatar_url TEXT;

-- Add constraint to ensure avatar_url is a valid URL (optional)
ALTER TABLE public.user_profiles 
...
-- Create index for user profile lookups
  ...
  IF OLD.avatar_url IS NOT NULL AND NEW.avatar_url IS NOT NULL AND OLD.avatar_url != NEW.avatar_url THEN
    ...
    BEGIN
      ...
      IF old_file_path IS NOT NULL THEN
        ...
        RAISE NOTICE 'Cleaned up old avatar file: %', old_file_path;
      END IF;
    EXCEPTION
      WHEN OTHERS THEN
...
-- Function to test avatar storage access
CREATE OR REPLACE FUNCTION public.test_avatar_access(user_id_param UUID)
RETURNS JSON AS $$
DECLARE
  result JSON;
BEGIN
  -- Test if user can read their profile
  PERFORM id FROM public.user_profiles WHERE id = user_id_param;
  ...
  result := json_build_object(
    'success', true,
    'message', 'Avatar access test passed',
...
COMMENT ON FUNCTION public.test_avatar_access(UUID) IS 'Test function to verify avatar storage access permissions';

-- Grant necessary permissions
GRANT USAGE ON SCHEMA storage TO authenticated;
GRANT ALL ON storage.objects TO authenticated;
GRANT ALL ON public.user_profiles TO authenticated;
GRANT EXECUTE ON FUNCTION public.test_avatar_access(UUID) TO authenticated;

-- Create a view for avatar debugging
CREATE OR REPLACE VIEW public.avatar_debug_view AS
SELECT 
  id,
  email,
  display_name,
  ...
  CASE 
    ...
    WHEN avatar_url ~ '^https?://.*' THEN 'Valid URL'
...
GRANT SELECT ON public.avatar_debug_view TO authenticated; 