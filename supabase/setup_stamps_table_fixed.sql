-- Fixed stamps table setup - handles existing table issues
...
-- Create user_stamps table with correct structure
...
CREATE INDEX idx_user_stamps_created_at ON public.user_stamps(created_at);
...
DROP FUNCTION IF EXISTS public.award_stamp_to_user;
...
SELECT 'Stamps table created successfully!' as message,
       'Columns: ' || string_agg(column_name, ', ') as columns
FROM information_schema.columns 
WHERE table_name = 'user_stamps' AND table_schema = 'public'
GROUP BY 1; 